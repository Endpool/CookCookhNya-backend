...
import domain.{IngredientId, IngredientNotFound, InternalServerError, StorageId}
...
def buyHandler(storageId: StorageId, ingredientIds: List[IngredientId]):
  ZIO[AuthenticatedUser & BuyEnv, InternalServerError | IngredientNotFound, Unit] = ZIO.serviceWithZIO[ShoppingListsRepo](
